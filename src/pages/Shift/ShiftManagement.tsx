...
const ShiftManagement: React.FC = () => {
  ...
  const fetchPatterns = async () => {
    try {
      //const data = await shiftService.GetShiftPatternsByOrganization(organizationID);
      //setPatterns(
        // data.map((p: any) => ({
        //   id: p.PatternID,
        //   name: p.PatternName,
 ...
 const payload = {
      ShiftID: formData.id || 0,           // For new or existing
      OrganizationID: organizationID,
      ShiftCode: formData.code,
      ShiftName: formData.name,
      StartTime: formData.start + ":00",   // HH:mm:ss
      EndTime: formData.end + ":00",       // HH:mm:ss
  ...
  const savePatternApi = async () => {
    ...
    try {
      //await shiftService.PostShiftPatternAsync(payload);
      await fetchPatterns();
      setShowModal(false);
    } catch (err) {
  ...
  const deletePatternApi = async (id: number) => {
    if (window.confirm("Delete this pattern?")) {
      try {
        //await shiftService.DeleteShiftPatternAsync(id);
        await fetchPatterns();
      } catch (err) {
    ...
    setAssignments((prev) => [
      {
        ...assignData,
        id: Date.now(),
        employeeName: staticEmployees.find((e) => e.id === Number(assignData.employeeId))?.name,
        shiftName: shifts.find((s) => s.id === Number(assignData.shiftId))?.name,
      },
      ...prev,
    ]);

    setAssignData({ employeeId: "", shiftId: "", effectiveFrom: "", effectiveTo: "", isRotational: false });
  };
  ...
  return (
    <div className="p-3 mt-3">
      ...
      <Nav variant="tabs" activeKey={activeTab} onSelect={(k) => setActiveTab(k || "shifts")}>
        <Nav.Item><Nav.Link eventKey="shifts">Shifts</Nav.Link></Nav.Item>
        <Nav.Item><Nav.Link eventKey="patterns">Shift Patterns</Nav.Link></Nav.Item>
        <Nav.Item><Nav.Link eventKey="employees">Employee Assignments</Nav.Link></Nav.Item>
        <Nav.Item><Nav.Link eventKey="assign">Assign Shift</Nav.Link></Nav.Item>
      </Nav>
              ...
              <Col md={4} key={p.id} className="mb-3">
                <Card className="shadow-sm">
                  <Card.Body>
                    ...
                    <div className="d-flex justify-content-end gap-2 mt-3">
                      <Button size="sm" variant="outline-primary" onClick={() => editPattern(p)}>
                        {Icon(BsPencilSquare)}
                      </Button>
                      <Button size="sm" variant="outline-danger" onClick={() => deletePatternApi(p.id)}>
                        {Icon(BsTrash)}
                      </Button>
                    </div>
                  </Card.Body>
          ...
          <Table className="table table-hover table-dark-custom">
            ...
            <tbody>
              {assignments.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-center text-muted">No assignments yet</td>
                </tr>
              )}
              {assignments.map((a) => (
                <tr key={a.id}>
            ...
            <Card.Body>
              ...
              <Row>
                <Col md={4}>
                  <Form.Group className="mb-3">
                    ...
                    <Form.Select id="employeeId" value={assignData.employeeId} onChange={handleAssignInput}>
              ...
              <Row>
                ...
                <Col md={4} className="mt-4">
                  <Form.Check id="isRotational" checked={assignData.isRotational} onChange={handleAssignInput} label="Rotational Shift" />
                </Col>
              </Row>
              <Button variant="primary" onClick={saveAssignment}>Assign Shift</Button>
            </Card.Body>
          </Card>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
            ...
            {modalType === "shift" ? (
              ...
              <>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control id="name" value={formData.name} onChange={handleInput} placeholder="4-On 3-Off" />
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>Description</Form.Label>
                  <Form.Control as="textarea" rows={3} id="desc" value={formData.desc} onChange={handleInput} placeholder="Describe the pattern" />
                </Form.Group>
              </>
            )}
          </Form>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={modalType === "shift" ? saveShiftApi : savePatternApi}>Save</Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
