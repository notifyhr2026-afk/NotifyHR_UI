...
const OrgSideMenu: React.FC<SideMenuProps> = ({ isOpen }) => {
  ...
  const buildMenuTree = (items: MenuItem[]): MenuItem[] => {
    ...
    items.forEach(item => {
      ...
      if (item.parentMenuID) {
        map.get(item.parentMenuID)?.subMenu?.push(node);
      } else {
        roots.push(node);
      }
    });
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      const data = await GetByUserIDAsync(UserID);
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {menu.map((item, idx) => {
            ...
            const isActive =
              ...
              (hasSub && item.subMenu!.some(sub => sub.link === location.pathname));

            const Wrapper: any = hasSub ? 'div' : Link;
            const wrapperProps = hasSub ? {} : { to: item.link };
            ...
            return (
              ...
              >
                <OverlayTrigger
                  placement="right"
                  overlay={!isOpen ? <Tooltip id={`tip-${idx}`}>{item.label}</Tooltip> : <></>}
                >
                  <Wrapper
                    {...wrapperProps}
                  ...
                  >
                    <i
                      className={`bi ${item.icon || 'bi-circle-fill'} me-2`}
                      style={{
                        fontSize: '1.4rem',
